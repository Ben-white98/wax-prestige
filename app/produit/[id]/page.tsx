...
import { Loader2, Minus, Plus, ShoppingCart, ArrowLeft, CheckCircle2 } from 'lucide-react';
...
export default function ProductPage() {
  ...
  const handleAddToCart = () => {
    if (product) {
      ...
      addItem({
        ...
        price: product.price,
        quantity,
        imageUrl: product.imageUrl || `https://picsum.photos/seed/${product.id}/1200/1600`
      });
  ...
  if (!product) return (
    <div className="container mx-auto px-4 py-32 text-center">
      <h1 className="text-3xl font-serif font-bold mb-4">Produit introuvable</h1>
      <p className="text-neutral-500 mb-8">Le pagne que vous recherchez n'existe pas ou a été supprimé.</p>
      <Link href="/catalogue" className="bg-amber-600 hover:bg-amber-700 text-white px-8 py-4 rounded-full font-medium transition-colors">
        Retour au catalogue
      </Link>
    </div>
  );
    ...
    <div className="container mx-auto px-4 py-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20">
        ...
        <div className="relative aspect-[3/4] rounded-3xl overflow-hidden bg-neutral-100 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 shadow-sm">
          <Image 
            src={product.imageUrl || `https://picsum.photos/seed/${product.id}/1200/1600`} 
            alt={product.name} 
            fill 
        ...
        <div className="flex flex-col justify-center">
          ...
          <div className="bg-neutral-50 dark:bg-neutral-900/50 p-6 rounded-2xl border border-neutral-200 dark:border-neutral-800 mb-10">
            <div className="flex items-center gap-2 mb-6">
              <CheckCircle2 className={`w-5 h-5 ${product.stock > 0 ? 'text-green-500' : 'text-red-500'}`} />
              <span className="font-medium">
                {product.stock > 0 ? `${product.stock} pièces disponibles en stock` : 'Actuellement indisponible'}
              </span>
            </div>
